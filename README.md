...
Apps live in `src/data/apps.json`. Each entry includes `id`, `name`, `slug`, descriptions, `logo_url`, `screenshots`, `tags`, `pricing`, `platforms`, `links`, `release_date`, optional `uses_ai` (badge “IA” en catálogo y ficha), and optional `why_exists` and `privacy_policy` (renders `/apps/[slug]/privacidad` and shows a “Política de privacidad” link on the app page).
